...
class ProductListCreateView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            category_obj = Category.objects.filter(id=request.data['category']).first()
        except Exception as e:
            return Response("categpory not avalible please create it first",status=status.HTTP_400_BAD_REQUEST)
        

        new_data = {
            "name": request.data['name'],
            "discription": request.data['description'],
        ...
        return Response("payload error",status=status.HTTP_400_BAD_REQUEST)
